# Copilot Instructions for Lu Doces
...
- Botão WhatsApp flutuante:
  ...
  ```
  ```css
  .whatsapp-float { opacity: 0; }
  .whatsapp-float.visible { opacity: 1; }
  ```
- Botão Churrasco flutuante:
  ```html
  <div class="floating-churrasco">...</div>
  ```
  ```css
  .floating-churrasco { opacity: 0; }
  .floating-churrasco.visible { opacity: 1; }
  ```

## Integrações
- Fontes e ícones via CDN (Google Fonts, Font Awesome)
- WhatsApp: link direto via `wa.me`

## Observações
- Não há backend, banco de dados ou frameworks JS.
- O site é estático e pode ser hospedado em qualquer serviço de arquivos estáticos.
- Para personalizar, edite diretamente os arquivos HTML, CSS e JS.

---

Seções ou padrões não documentados? Solicite exemplos ou esclarecimentos para manter este guia atualizado.
